			...
			<div class="container">
				...
				<div class="row">
					<div class="pricing centered">
						<?php 
							$count = R::count('type_ticket');
							for ($i=0; $i < $count; $i++) { 
								$ticket = R::load('type_ticket', $i + 1);
								echo 
								"<div class=col-md-3>
									<div class='price-box"; if ( strcmp($ticket->name_ticket,'Pro') == 0 ) echo " popular'"; else echo "'"; echo ">
										<h2 class=pricing-plan>$ticket->name_ticket</h2>
										<div class=price><sup class=currency>₽</sup>$ticket->price<small>/$ticket->name </small></div>
										...
										if( !isset($_SESSION['logged_user']) )
										echo "<button id=openD class='btn btn-default podval' data-toggle=modal data-target=#firstModal>Select Plan</button>";
					    				else 
					   					echo "<button id=openD class=btn btn-default podval data-toggle=modal data-target='#secondModal$ticket->id'>Select Plan</button>"; 
								echo "</div>
								</div>

								<div class='pricing centered' id=secondModal>
								    <div class='modal fade' id='secondModal$ticket->id'>
								 			...
								 			<div class=modal-content>
								 				...
								 				<div class=modal-body>
													<form method=POST class=form-horizontal>
														<span class=heading>Покупка '$ticket->name_ticket' абонемент</span>
														<div class=form-group>
															<h3>Стоимость<div class=price><sup class=currency>₽</sup>$ticket->price<small>/месяц</small></div></h3>
															<span>"; echo $_SESSION['logged_user']->name.' '.$_SESSION['logged_user']->surname; echo "</span>
														</div>
														<div class=form-group>
															<button name=buy type=submit class=btn btn-default value=$ticket->name>Купить</button>
														</div>
													</form>
								 				</div>
